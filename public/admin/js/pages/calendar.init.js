...
! function (e, t) {
    ...
    function nova_intervencao(e){
        var _token = $('meta[name="_token"]').attr('content');
        var inicio = new Date(e.start.getTime());
        if(String(inicio.getMonth()).length == 1){
            var mes = "0" + (inicio.getMonth() + 1);
        }else{
            var mes = inicio.getMonth() + 1;
        }
        inicio = inicio.getFullYear() + "-" + mes + "-" + inicio.getDate() + " " + inicio.getHours() + ":" + inicio.getMinutes() + ":" + inicio.getSeconds();
        
        var fim = new Date(e.end.getTime());
        if(String(fim.getMonth()).length == 1){
            mes = "0" + (fim.getMonth() + 1);
        }else{
            mes = fim.getMonth() + 1;
        }
        fim = fim.getFullYear() + "-" + mes + "-" + fim.getDate() + " " + fim.getHours() + ":" + fim.getMinutes() + ":" + fim.getSeconds();
        console.log(fim);
        ...
        $.ajax({
            url: '/dashboard/calendario/intervencao/nova',
            type: 'POST',
            data: {
                identificador: String(chance.guid()),
                assunto: e.title,
                inicio: inicio,
                fim: fim,
                observacao: e.location,
                situacao: e.calendarId
            },
            dataType: 'JSON',
    ...
    function atualiza_intervencao(e, t){
        var _token = $('meta[name="_token"]').attr('content');
        var id = e.id;
        var data = {};
        if(t.title){
            data.assunto = t.title;
        }
        if(t.start){
            var inicio = new Date(t.start.getTime());
            if(String(inicio.getMonth()).length == 1){
                var mes = "0" + (inicio.getMonth() + 1);
            }else{
                var mes = inicio.getMonth() + 1;
        ...
        if(t.end){
            var fim = new Date(t.end.getTime());
            if(String(fim.getMonth()).length == 1){
                var mes = "0" + (fim.getMonth() + 1);
            }else{
                var mes = fim.getMonth() + 1;
            }
            fim = fim.getFullYear() + "-" + mes + "-" + fim.getDate() + " " + fim.getHours() + ":" + fim.getMinutes() + ":" + fim.getSeconds();
        ...
        $.ajax({
            url: '/dashboard/calendario/intervencao/atualizar/' + id,
            type: 'POST',
            data: data,
            dataType: 'JSON',
            success: function(data) {
    ...
    function remove_intervencao(e){
        ...
        $.ajax({
            url: '/dashboard/calendario/intervencao/remover/' + id,
            type: 'GET',
            dataType: 'JSON',
            success: function(data) {
                // console.log(data);
